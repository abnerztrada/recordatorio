...
class correo
{
  ...
  public function correo_envio(){
    //Query del curso con fechas
      $query = "Select c.id, c.shortname, c.fullname, DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(c.startdate, '%Y-%m-%d %H:%i'), INTERVAL -5 HOUR),'%d/%m/%Y %H:%i') AS fechainicio,
      DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(c.enddate, '%Y-%m-%d %H:%i'), INTERVAL -5 HOUR),'%d/%m/%Y %H:%i') AS fechafinal,
      DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(c.enddate, '%Y-%m-%d %H:%i'), INTERVAL -45 HOUR),'%d/%m/%Y %H:%i') AS fecharecordatorio,
      DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(c.enddate, '%Y-%m-%d %H:%i'), INTERVAL -21 HOUR),'%d/%m/%Y %H:%i') AS fecharecordatoriov,
      DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(c.enddate, '%Y-%m-%d %H:%i'), INTERVAL -69 HOUR),'%d/%m/%Y %H:%i') AS fecharecordatoriom
      from mdl_course c where c.visible = 1 and c.id = 225"; /*and c.id = 9 or c.id = 3*/
      ...
      foreach ($result as $it) {
        ...
        $fecharecordatorioJueves = $it->fecharecordatorio;
        $fecharecordatorioViernes = $it->fecharecordatoriov;
        $fecharecordatorioMiercoles = $it->fecharecordatoriom;
        ...
        $result3 = $DB->get_records_sql($query3);
          ...
          foreach ($result3 as $it3) {
            ...
            // echo "-------------------------------";
            // echo "fechaLunes = ".$fechaLunes."\n";
            // echo "fechaInicio = ".$fechaInicio."\n";
            // echo "FechaAct = ".$fechaAct."\n";
            // echo "fecha jueves = ".$fecharecordatorioJueves."\n";
            // echo "fecha viernes = ".$fecharecordatorioViernes."\n";
            // echo "fecha fin = ".$fechaFinal."\n";
            // echo "fecha miercoles = ".$fecharecordatorioMiercoles."\n";
            // echo "------------------------------------------------";
            ...
            }else if($fecharecordatorioJueves == $fechaAct){
              if($estados == 'no iniciado' || $estados == 'iniciado'){
                $email = email_to_user($emailuser,'laUcmi','Recuerda completar tu curso'.$subject,$string1);
                echo "Correo enviado";
              }
            }else if($fecharecordatorioViernes == $fechaAct){
              if($estados == 'no iniciado' || $estados == 'iniciado'){
                $email = email_to_user($emailuser,'laUcmi','Recuerda completar tu curso'.$subject,$string1);
                echo "Correo enviado";
              }
            }else if($fecharecordatorioMiercoles == $fechaAct){
              if($estdos == 'no iniciado' || $estados == 'iniciado' ){
                $email = email_to_user($emailuser,'laUcmi','Recuerda completar tu curso'.$subject,$string1);
                echo "Correo enviado";
              }
            }else{
